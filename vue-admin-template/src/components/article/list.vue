  ...
  <div class="article">
    ...
    <div class="article-load">
      <button class="loadmore-button" :disabled="article.fetching || !isCanLoadMore" @click="$emit('loadmore')">
        ...
        <div class="text">
          <span v-if="!article.fetching && isCanLoadMore">加载更多</span>
          <span v-else-if="article.fetching && isCanLoadMore">正在获取</span>
          <span v-else-if="!isCanLoadMore">已经没有了</span>
        </div>
      </button>
    </div>
  </div>
...
<script>
  ...
  import { getArticleDetailRoute } from '@/api/article'
